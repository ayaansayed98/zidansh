...
import { Trash2, Info, Mail, Phone, X, CreditCard, Smartphone, Truck, Banknote } from 'lucide-react';
...
const CheckoutForm: React.FC<CheckoutFormProps> = ({
  ...
  // Indian cities database with state mapping
  ...
  const quickPaymentMethods = [
    { id: 'card', name: 'Card', description: 'Credit/Debit', icon: CreditCard },
    { id: 'upi', name: 'UPI', description: 'Paytm/PhonePe', icon: Smartphone },
    { id: 'cash', name: 'Cash', description: 'On Delivery', icon: Banknote },
    { id: 'netbanking', name: 'Net Banking', description: 'Online Banking', icon: Truck }
  ];
  ...
  const upiApps = [
    {
      id: 'gpay',
      name: 'Google Pay',
      logo: <img src="/zidansh img/realimg/gpay logo.png" alt="Google Pay" className="w-full h-full object-contain" />
    },
    {
      id: 'phonepe',
      name: 'PhonePe',
      logo: <img src="/zidansh img/realimg/phonepay logo.png" alt="PhonePe" className="w-full h-full object-contain" />
    },
    {
      id: 'paytm',
      name: 'Paytm',
      logo: <img src="/zidansh img/realimg/paytm logo.png" alt="Paytm" className="w-full h-full object-contain" />
    },
    {
      id: 'amazonpay',
      name: 'Amazon Pay',
      logo: <img src="/zidansh img/realimg/amazonpay logo.png" alt="Amazon Pay" className="w-full h-full object-contain" />
    },
    {
      id: 'bhim',
      name: 'BHIM UPI',
      logo: <img src="/zidansh img/realimg/bhimupi logo.png" alt="BHIM UPI" className="w-full h-full object-contain" />
    }
  ];

  // Calculate all charges and totals with proper memoization
  const orderCalculations = useMemo(() => {
    // Calculate subtotal from cart items or use provided totalAmount
    const subtotal = totalAmount ?? cartItems.reduce((sum: number, item: CartItem) =>
      sum + (item.price * (item.quantity || 1)), 0);

    // Calculate delivery charge (free for orders over 1000)
  ...
  const handleUpiAppSelect = useCallback((app: string) => {
  ...
  const validateFormData = (): boolean => {
    // prepare data for validation including payment method
    ...
    // Map 'paymentMethod' error to 'payment' key if it exists, to match UI
  ...
  // Calculate final total function
  ...
  return (
    <div className={`${isModal ? 'bg-white rounded-lg shadow-xl max-w-2xl w-full max-h-[80vh] flex flex-col' : 'h-full flex flex-col'}`}>
      <div className="relative w-full max-w-5xl mx-auto bg-white shadow-xl flex flex-col h-[90vh] my-8 rounded-lg overflow-hidden">
        ...
        <div className="flex-1 flex flex-col md:flex-row overflow-hidden">
          ...
          <div className="w-full md:w-2/5 lg:w-96 bg-gray-50 p-4 border-r border-gray-200 overflow-y-auto">
            ...
            <div className="space-y-2">
              {(() => {
                // Group cart items by product ID and size
        ...
        <div className="flex-1 overflow-y-auto">
          <form onSubmit={handleSubmit} className="p-4 space-y-4">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                {quickPaymentMethods.map((method) => (
                  <button
                    ...
                    className={`p-4 border rounded-lg text-center hover:border-pink-500 transition-colors ${selectedPaymentMethod === method.id
            ...
            {selectedPaymentMethod === 'upi' && (
              <div>
                <h4 className="font-medium text-gray-900 mb-3">Select UPI App</h4>
                <div className="grid grid-cols-3 md:grid-cols-6 gap-3">
                  {upiApps.map((app) => (
                    <button
                      ...
                      className={`p-3 border rounded-lg text-center hover:border-pink-500 transition-colors ${selectedUpiApp === app.id
                          ? 'border-pink-500 bg-pink-50'
                          : 'border-gray-200'
                        }`}
                    >
        ...
        {/* Fixed Submit Button */}
        <div className="relative bg-white border-t border-gray-200 p-6 z-10">
          <div className="flex gap-3">
            ...
            <button
              ...
              disabled={isProcessing}
              className="flex-1 px-8 py-4 bg-black text-pink-700 rounded-lg hover:bg-gray-900 hover:text-pink-600 transition-all disabled:opacity-50 disabled:cursor-not-allowed font-bold text-xl shadow-2xl border-2 border-black hover:border-gray-900 transform hover:scale-105 relative z-20"
            >
              {isProcessing ? (
